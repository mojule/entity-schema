...
const SchemaMapResolver = ( schemaMap : ISchemaMap ) : ( id : string ) => JSONSchema4 => {
  const resolver = ( id: string ) => schemaMap[ id ]
...
export const SchemaCollection = ( schemas : IAppSchema[] ) => {
  ...
  const schemaCollection = {
    ...
    interfaceSchema: ( title: string ) => {
      ...
      const schema = schemaCollection.normalize( title )
    ...
    mongooseSchema: ( title: string ) => {
      ...
      const schema = <IEntitySchema>schemaCollection.normalize( title )
    ...
    filterEntity: ( title: string, entity: EntityModel ) => {
      assertEntityTitle( title )

      const schema = <IEntitySchema>schemaCollection.normalize( title )

      return filterEntityBySchema( entity, schema )
    },
