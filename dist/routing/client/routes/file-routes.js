...
exports.FileRoutes = (resolverNames) => {
    ...
    return {
        '/files': async (_req, res) => {
            const links = await ids_to_links_1.entityTypesToLinks(ids, '/files');
        ...
        '/files/disk-file': async (_req, res) => {
            ...
            const links = await ids_to_links_1.entityTypesToLinks(ids, '/files', 'disk-file');
            const filesNav = templates_1.AnchorNav(links);
            filesNav.classList.add('secondary-nav');
        ...
        '/files/image-file': async (_req, res) => {
            ...
            const links = await ids_to_links_1.entityTypesToLinks(ids, '/files', 'image-file');
            const filesNav = templates_1.AnchorNav(links);
            filesNav.classList.add('secondary-nav');
        ...
        '/files/zip-file': async (_req, res) => {
            ...
            const links = await ids_to_links_1.entityTypesToLinks(ids, '/files', 'zip-file');
            const filesNav = templates_1.AnchorNav(links);
            filesNav.classList.add('secondary-nav');
